...
### r2 复审 → 再修订（`01-review-r2.md`，MAJOR）

**★★★ r2 证明：r1 为补洞新建的两把锁（偏置修法、`criterion_matrix`），两把都在物理层瞎，而所有机械 CRIT 门全过 —— 门查「在场」，不查「算对」。** 这正是本仓库最贵的元教训（P18 + 「拆掉一把失明的锁，换上的新锁多半也瞎」），这次复现在**我自己的 r1 修订**上。已修：

- **H2'（MAJOR，判据里的 P17）**：C2 容差从**算错的误差传播**来 —— $\bar h\propto A^2$，独立测 $A$ 的 10% 进 $\bar h$ 是 **20%**（不是当线性的 10%），真实预言本底求积 ≈**22%**，不是 15%。**修**：C2 容差 15%→22%（源码写清 $\propto A^2$）；**robustness 改扫主导量 $A$**（原来只扫 $e$，对主导不确定度结构性失明），margin 1.06（薄但诚实）；$\varepsilon^*$ 去虚高（0.156→0.227）。
- **H4'（MINOR，教训 4 复发）**：H6 的 $e$ 因子进了正文没进契约 —— `model-spec.json` 的 $\Pi_f$ 补回，0.3183→0.4502，与 eq.2 一致。
- **★ H1'（MAJOR，诚实边界，未完全修 —— 记为已知局限）**：加偏置后力含 $1f{+}2f$ ⟹ 扫频共振峰**重新二义**：固定-自由峰在 $\{7,14\}$ kHz、自由-自由在 $\{14,28\}$ kHz，**14 kHz 处两种 BC 都有峰**。⟹ **A-1 的退化特征「峰位 14 vs 28」被谐波污染，C1 判据（内嵌把 peak 硬编码成本征频率）绕过了含谐波的真实测量链**。且「$1f$ 主导」需 $H_b\gg H_{ac}/4$，而 S-1 的软磁 Ni-Zn 低剩磁，此断言在选定材料上可疑。**诚实结论**：**峰位单读数不足以定 BC**；干净的退化特征需**模式形状（多点测振）或完整谐波图样**，或加大偏置。这是一条真实的物理限制，本轮不假装 C1 覆盖了它 —— 明写为局限。

> **★ 元结论（这次练习最贵的收获）**：即便是**有全仓库上下文、满脑子这些元教训**的作者，r1 补洞时照样建了两把瞎锁，只有 fresh 对抗审稿抓得到 —— 而机械门全绿。**「没有不动点，流程即交付物」**（electrical-damping 8 轮停环那条）在一道新题、在我自己身上又复现了一遍。⟹ 到此**停环**（与 electrical-damping 同一裁定）：真 bug（H2'/H4'）已廉价修掉，H1' 明写为诚实局限，继续 r3/r4 只会再复现同一条元教训。而 H1' 又一次印证 `02-correspond.md`：真实团队用剩磁偏置 + 实测共振谱避开的，正是无上下文独立分析看不清的地方。
