...
# ★ r7 审稿 H1：判死悬崖必须在噪声外（delta_max ≥ budget），否则噪声一涨就把偏心推过悬崖、
#   正确模型被自己的判据误杀 —— 而 `bad` 以前从不读它，散文里的「安全裕度」没有门。
bad |= (delta_max is not None and delta_max < NOISE_BUDGET)
...
out = {
    ...
    # ★ r7 审稿 H2：源码 sha256 戳 —— DESYNC 门重算比对，抓「改源码忘重跑」（教训 19）。
    #   规范化行尾（\r\n→\n）后再哈希，免得跨平台 checkout 换行尾就误报脱钩。
