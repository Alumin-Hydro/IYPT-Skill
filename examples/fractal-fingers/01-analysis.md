...
即 **$\lambda^\ast/h \propto \mathrm{Ca}^{-1/2}$（指数 $-1/2$）**。最软的软肋是 **A-2**（醇与水互溶 ⇒ 界面张力 $\gamma$ 是否真的存在、是否足以选出 $\lambda^\ast$）与 **A-3**（丙烯漆剪切变稀）。分形维数 $D\approx1.71$ 是 **DLA 普适类的文献结论**（不是本模型的推导，见 §10）。
...
- **失效边界**：$h$ 增大（扫描上端 $h=1.0$ mm，$h/\lambda^\ast\to0.5$）或黏度比很大使 $\lambda^\ast$ 变小时，三维流动与竖直方向的压强分布不可再忽略
- **分级**：`LOAD-BEARING`
...
**数据坍缩预言**：把不同 $(U,\mu_2,\gamma,h)$ 组合的实测指宽画成 $\lambda^\ast/h$ vs $\mathrm{Ca}$，**应坍缩到同一条斜率 $-1/2$ 的直线**（log-log）。这是本报告杀伤力最大的图（→ F-2）。**若不坍缩**，多半是 A-3（剪切变稀，$\mathrm{Ca}$ 不再是唯一控制组）或 A-2（无 $\gamma$）破了。
...
| 预测 | 内容 | 怎么证伪 |
|---|---|---|
| **P1** | $\lambda^\ast/h\propto\mathrm{Ca}^{-1/2}$，**指数 $=-0.50$，零自由参数** | 扫 $U$（或 $\mu_2$），FFT 提 $\lambda^\ast$，测 $\log\lambda^\ast$–$\log\mathrm{Ca}$ 斜率。测得 $-1$ ⇒ naive-B（毛细平衡）；$0$ ⇒ naive-A（只由 $h$ 定）。模型死。（criterion K1） |
| **P2** | $\lambda^\ast=\pi h\sqrt{\gamma/(U(\mu_2-\mu_1))}=2.12$ mm（给定独立测的 $\gamma$）⇒ $N\approx9$ | 数指的条数。偏 $1/\sqrt3$（$\approx-42\%$）⇒ 把临界波长当了指宽（onset-C）。（criterion K2） |
| **P3** | 不同 $(U,\mu_2,\gamma,h)$ 的 $\lambda^\ast/h$ vs $\mathrm{Ca}$ **坍缩到一条斜率 $-1/2$ 的线** | 画坍缩图。不坍缩 ⇒ 漏了一个组（多半 A-3 剪切变稀，或 A-2 无 $\gamma$）。（F-2） |
| **P4** | $N=(2R/h)\sqrt{\mathrm{Ca}(1-r)}\propto R\sqrt{\mathrm{Ca}}$ | 扫 $R$，数 $N$。斜率 $\ne\tfrac12$（对 $\mathrm{Ca}$）或 $\ne1$（对 $R$）⇒ 查模型 |
...
→ `handoff/model-spec.json`（Skill 2 的输入契约）；`01-criteria/criterion_matrix.py` → `matrix.json`（判据双向表，已内嵌进契约的 `criterion_matrix`）。
