...
TOL = {
    ...
    "K2": (0.20,
           "**绝对值/一参**判据（绝对值含 γ、需实测 ⟹ 非零参；零参的是指数 K1）——预言侧不确定度主导（r1 审稿 H4 订正）：\n"
           "  λ* ∝ h·γ^(1/2)·U^(−1/2)·(μ2−μ1)^(−1/2) ⟹ 灵敏度系数 h→1、γ/U/μ2→½。\n"
           "  δh/h=10%、**δγ/γ=30%（无表值、需实测；与 §9 统一，不再是旧稿的 ±15%）**、δU/U=10%、δμ2/μ2=10%。\n"
           "  **求和方**（各项独立）：√(10² + (½·30)² + (½·10)² + (½·10)²) = √(10²+15²+5²+5²) ≈ **19.4%** ⟹ 门槛 **20%**。\n"
